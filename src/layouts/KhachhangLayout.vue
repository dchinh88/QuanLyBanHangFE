...
const { fetchKhachhang, query } = useKhachhang();
...
const getAllKhachhang = async () => {
  const res = await fetchKhachhang();
  khachhang.value = res.data;
...
const searchKhachhang = computed(() => {
  const keyword = search.value.toLowerCase();
  return khachhang.value.filter((p) => p.hoten.toLowerCase().includes(keyword));
});
...
const deleteLoaisanpham = async () => {
  try {
    ...
    const response = await serviceKhachhang.deleteKhachhang(id.value);
    ...
    <v-row class="mr-2">
      <v-col cols="12">
        <v-table style="border-radius: 12px 12px 0 0">
          ...
          <tbody>
            <tr v-for="(item, i) in searchKhachhang" :key="i">
              <td
              ...
              <td class="text-left">
                <v-btn
                  ...
                  size="small"
                  flat
                  @click="(dialogEdit = true), (currentKh = item), (idKh = item.id)"
                >
