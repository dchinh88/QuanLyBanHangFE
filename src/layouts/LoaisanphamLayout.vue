...
import { useLoaisanpham } from './components/loaisanpham/loaisanpham.store';
import { serviceLoaisanpham } from './components/loaisanpham/loaisanpham';
import AddLoaisanpham from '../components/loaisanpham/AddLoaisanpham.vue';
import EditLoaisanpham from '../components/loaisanpham/EditLoaisanpham.vue';

const { fetchLoaisanpham, query } = useLoaisanpham();
...
const getAllLoaisanpham = async () => {
  const res = await fetchLoaisanpham();
  loaisanpham.value = res.data;
...
const searchLoaisanpham = computed(() => {
  ...
  return loaisanpham.value.filter((p) =>
    p.tenloaisanpham.toLowerCase().includes(keyword),
  );
});
...
const deleteLoaisanpham = async () => {
  try {
    ...
    const response = await serviceLoaisanpham.deleteLoaisanpham(id.value);
            ...
            <tr v-for="(item, i) in searchLoaisanpham" :key="i">
              ...
              <td class="text-left">
                <v-btn
                  ...
                  size="small"
                  flat
                  @click="(dialogEdit = true), (currentLsp = item), (idLsp = item.id)"
                >
